...
class StacticsClientTest < Minitest::Test
  ...
  def test_tracks_single_event
    ...
    client = Stactics::Client.new(api_key: "st_secret_test", host: "https://api.example.test", transport: transport)
    ...
    assert_equal "Bearer st_secret_test", transport.requests.first.dig(:headers, "Authorization")
    assert_equal(
      {
        "event_type" => "signup",
        "user_id" => "user_123",
  ...
  def test_sends_batches
    ...
    client = Stactics::Client.new(api_key: "st_secret_test", transport: transport)
